...
| Comando/Leadership | ✅🟡 | Bonus nello stesso esagono ✅; raggio di Comando multi-esagono per il **fuoco** (`Combat.fire_group`) ✅ e per la **mossa di gruppo** ✅ (un ordine di Mossa dato a un leader attiva sé stesso e le unità idonee entro il suo Comando; il giocatore le muove una alla volta, `Game._form_move_group`). Resta l'attivazione di gruppo per Avanzata/Recupero/Rotta. |
